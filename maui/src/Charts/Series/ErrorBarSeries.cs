...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	/// ***
	/// </example>
	public partial class ErrorBarSeries : XYDataSeries, IDrawCustomLegendIcon
	{
		...
		/// <value>It accepts the <see cref="ErrorBarLineStyle"/> values.</value>
		/// <example>
		/// # [Xaml](#tab/tabid-20)
		/// <code><![CDATA[
		///     <chart:SfCartesianChart>
		...
		/// </summary>
		/// <remarks>Null values are invalid.</remarks>
		/// <value>It accepts the <see cref="ErrorBarCapLineStyle"/> values.</value>
		/// <example>
		/// # [Xaml](#tab/tabid-24)
		/// <code><![CDATA[
		///     <chart:SfCartesianChart>
		...
		internal override void GenerateSegments(SeriesView seriesView)
		{
			...
			{
				if (CreateSegment() is ErrorBarSegment segment)
				{
					segment.Series = this;
		...
		internal override bool IsMultipleYPathRequired
		{
			get
			{
				bool yPathDecision = Type is ErrorBarType.Custom;
		...
		internal override void RemoveData(int index, NotifyCollectionChangedEventArgs e)
		{
			...
			else if (XValues is IList<string> list)
			{
				list.RemoveAt(index);
				PointsCount--;
			}
		...
		void GeneratePoints()
		{
			...
			{
				GeneratePoints([YBindingPath, HorizontalErrorPath, VerticalErrorPath], YValues, HorizontalErrorValues!, VerticalErrorValues!);
			}
			else if (YBindingPath != null && HorizontalErrorPath != null)
			{
				GeneratePoints([YBindingPath, HorizontalErrorPath], YValues, HorizontalErrorValues!);
			}
			else if (YBindingPath != null && VerticalErrorPath != null)
			{
				GeneratePoints([YBindingPath, VerticalErrorPath], YValues, VerticalErrorValues!);
			}
