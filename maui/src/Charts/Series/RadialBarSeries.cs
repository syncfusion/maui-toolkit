...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	/// ***
	/// </example>
	public partial class RadialBarSeries : CircularSeries, IDrawCustomLegendIcon
	{
		#region Internal Fields

		internal int _visibleSegmentCount;
		...
		internal override void GenerateSegments(SeriesView seriesView)
		{
			if (YValues != null && ActualData != null)
			{
				...
				for (int i = 0; i < YValues.Count; i++)
				{
					...
					if (i < _segments.Count && _segments[i] is RadialBarSegment segment)
					{
						segment.SetData(_startAngle, _endAngle, _yValue);
					}
					else
					{
						...
						radialBarSegment.VisibleSegmentIndex = _visibleSegmentCount;
		...
		internal override TooltipInfo? GetTooltipInfo(ChartTooltipBehavior tooltipBehavior, float x, float y)
		{
			...
			if (_segments[index] is not RadialBarSegment radialBarSegment)
			{
				return null;
			}

			float segmentRadius = RadialBarSeries.GetTooltipRadius(radialBarSegment);
		...
		static float GetTooltipRadius(RadialBarSegment radialBarSegment)
		{
		...
		void UpdateTrackProperties(object property)
		{
			...
			{
				var segment = (RadialBarSegment)_segments[i];
				switch (property)
				{
					case double:
						{
