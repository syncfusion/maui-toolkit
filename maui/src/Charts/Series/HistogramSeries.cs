...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	/// ***
	/// </example>
	public partial class HistogramSeries : XYDataSeries, IDrawCustomLegendIcon
	{
		#region Fields

		const int DistributionPointsCount = 100;
		static readonly double SqrtDoublePI = Math.Sqrt(2 * Math.PI);
		List<double> _distributionYValues;
		List<double> _distributionXValues;
		readonly DistributionSegment _distributionSegment;
#if __ANDROID__
...
#nullable enable
		...
		internal override TooltipInfo? GetTooltipInfo(ChartTooltipBehavior tooltipBehavior, float x, float y)
		{
			...
			if (tooltipInfo != null)
			{
				if (_segments[index] is HistogramSegment histogramSegment)
				{
		...
		internal override double GetDataLabelPositionAtIndex(int index)
		{
			...
			if (_segments?[index] is HistogramSegment segment)
			{
		...
		{
			GetHistogramMeanAndDeviation(dataPoints, out var mean, out var deviation);
			double min = start;
		...
		HistogramSegment? GetHistogramSegment(double position, double interval, int pointsCount, double bottomValue, int i, int index, List<object> data)
		{
			...
			if (histogramSegment != null)
			{
				...
				histogramSegment.SetData([position, position + interval, pointsCount, bottomValue, position, i]);
				...
				_segments.Add(histogramSegment);
			}
