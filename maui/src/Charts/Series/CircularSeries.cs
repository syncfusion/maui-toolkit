...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public abstract class CircularSeries : ChartSeries
	{
		...
		internal bool _isClockWise;

		bool _isIncreaseAngle;
		...
		internal double GetAngleDifference()
		{
			...
			angleDifference = Math.Abs(angleDifference).Equals(360) ? _isClockWise ? Math.Abs(angleDifference - 0.0001) : angleDifference + 0.0001 : angleDifference;
		...
		internal void ChangeIntersectedLabelPosition()
		{
			if (DataLabelSettings.SmartLabelAlignment == SmartLabelAlignment.Shift)
			{
				...
				CircularSeries.ArrangeDataLabelsWithoutOverlap(LeftPoints, false);
				CircularSeries.ArrangeDataLabelsWithoutOverlap(RightPoints, true);
			}
			else if (DataLabelSettings.SmartLabelAlignment == SmartLabelAlignment.Hide)
			{
				for (int i = 0; i < _segments.Count - 1; i++)
				{
					if (_segments[i] is not PieSegment currentSegment || _segments[i + 1] is not PieSegment nextSegment)
					{
						return;
					}
					...
					if ((currentSegment.IsVisible && currentSegment.LabelRect.IntersectsWith(nextSegment.LabelRect)) ||
						(!currentSegment.IsVisible && currentSegment.Index > 0 &&
						 _segments[currentSegment.Index - 1] is PieSegment previousSegment &&
						 previousSegment.LabelRect.IntersectsWith(nextSegment.LabelRect)))
					{
						_segments[nextSegment.Index].IsVisible = false;
					}
		...
		internal static PointF calculateOffset(double degree, double radius, PointF center)
		{
			float radian = CircularSeries.degreesToRadians((float)degree);
		...
		{
			return segment.LabelPositionPoint.X > 0 && (segment.LabelPositionPoint.X + (segment.LabelRect.Width / 2)) < AreaBounds.Right && segment.LabelPositionPoint.Y > 0;
		}
		...
		void DrawConnectorLine(ICanvas canvas, ChartSegment dataLabel)
		{
			...
			if (dataLabel is not PieSegment pieSegment || connectorLineStyle.StrokeWidth < 0)
			{
				return;
			}
		...
		PointF GetActualCenter(float x, float y, float radius)
		{
			...
			float[] regions = [-630, -540, -450, -360, -270, -180, -90, 0, 90, 180, 270, 360, 450, 540, 630];
			List<int> region = [];
		...
		static void ArrangeDataLabelsWithoutOverlap(List<PieSegment> points, bool isRightSide)
		{
		...
		void ArrangeRightSidePoints()
		{
			...
			PieSegment? lastPoint = RightPoints?.Count > 1 ? RightPoints[^1] : null;
			...
			for (int i = RightPoints.Count - 2; i >= 0; i--)
			{
				...
				if (CircularSeries.IsOverlapWithNext(currentPoint, RightPoints, i) && currentPoint.IsVisible ||
		...
		static bool IsOverlapWithNext(PieSegment point, List<PieSegment> points, int pointIndex)
		{
			for (int i = pointIndex; i < points.Count; i++)
			{
				if (i != points.IndexOf(point) && points[i].IsVisible &&
					...
					CircularSeries.IsOverlap(point.LabelRect, points[i].LabelRect))
				{
					return true;
				}
			}
		...
		void IncreaseAngle(PieSegment currentPoint, PieSegment nextPoint, bool isRightSide)
		{
			...
			if (isRightSide)
			{
				...
				while (CircularSeries.IsOverlap(currentPoint.LabelRect, nextPoint.LabelRect))
				{
					...
					if (CircularSeries.IsOverlap(currentPoint.LabelRect, nextPoint.LabelRect) &&
		...
		void ArrangeLeftSidePoints()
		{
			...
			for (int i = 1; i < LeftPoints?.Count; i++)
			{
				...
				if (IsOverlapWithPrevious(currentPoint) && currentPoint.IsVisible || !(currentPoint.SegmentNewAngle! < 270))
				{
					...
					if (!_isIncreaseAngle)
					{
						for (int k = i; k > 0; k--)
						{
							...
							{
								if (!double.IsNaN(LeftPoints[index]._isLabelUpdated) &&
				...
				{
					if (angleChanged && previousPoint != null && (previousPoint._isLabelUpdated) == 1)
					{
						startFresh = true;
					}
				}
		...
		void DecreaseAngle(PieSegment currentPoint, PieSegment previousPoint, bool isRightSide)
		{
			...
			if (isRightSide)
			{
				while (CircularSeries.IsOverlap(currentPoint.LabelRect, previousPoint.LabelRect))
				{
		...
		{
			if (this is not PieSeries pieSeries)
			{
				return;
			}
			...
			double dataLabelMidRadius = radius + (radius * currentPoint._connectorLength);
			PointF startPoint = CircularSeries.calculateOffset(newAngle, radius, center!);
			PointF endPoint = CircularSeries.calculateOffset(newAngle, dataLabelMidRadius, center!);
			...
			{
				currentPoint.XPoints[2] = endPoint.X + currentPoint._connectorBendLineLength;
				currentPoint.YPoints[2] = endPoint.Y;
				rect = new Rect(currentPoint.XPoints[2] + LabelGap + currentPoint.LabelRect.Width / 2, currentPoint.YPoints[2], currentPoint.LabelRect.Width, currentPoint.LabelRect.Height);
			}
			else
			{
				currentPoint.XPoints[2] = endPoint.X - currentPoint._connectorBendLineLength;
				currentPoint.YPoints[2] = endPoint.Y;
				rect = new Rect(currentPoint.XPoints[2] - currentPoint.LabelRect.Width / 2 - LabelGap, currentPoint.YPoints[2], currentPoint.LabelRect.Width, currentPoint.LabelRect.Height);
			}

			currentPoint.LabelRect = rect;
			currentPoint.LabelPositionPoint = new PointF((float)rect.X, (float)rect.Y);

			endPoint = CircularSeries.calculateOffset(currentPoint.SegmentMidAngle, dataLabelMidRadius, center!);
		...
		{
			if (pieSegment == null || pieSegment.LabelContent == null)
			{
				return;
			}
			...
			{
				label = CircularSeries.GetTrimmedText(label, ref x, right - clipRect.Left, DataLabelSettings.LabelStyle, pieSegment.IsLeft, ref labelWidth);
				...
				{
					pieSegment.XPoints[2] = (float)labelRect.Right + LabelGap;
				}
			}

			if (right > clipRect.Right)
			{
				label = CircularSeries.GetTrimmedText(label, ref x, clipRect.Right - left, DataLabelSettings.LabelStyle, pieSegment.IsLeft, ref labelWidth);
				var labelRect = new Rect(x - labelWidth / 2, point.Y - pieSegment.LabelRect.Height / 2, labelWidth, pieSegment.LabelRect.Height);

				if (pieSegment.XPoints.Count > 2)
				{
					pieSegment.XPoints[2] = (float)labelRect.Left - LabelGap;
				}
			}
		...
		static string GetTrimmedText(string text, ref float x, double labelsExtent, ChartLabelStyle labelStyle, bool isLeft, ref double labelWidth)
		{
			string label = text;
			...
			if (oldSize > labelsExtent)
			{
				...
				{
					label = string.Concat(text.AsSpan(0, i), "...");
