namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public partial class PolarLineSeries : PolarSeries
	{
				...
				for (int i = 0; i < PointsCount; i++)
				{
					...
					else
					{
						if (i == PointsCount - 1)
						{
							if (IsClosed == true)
							{
								CreateSegment(seriesView, [xValues[i], YValues[i], xValues[0], YValues[0]], i);
							}
							else
							{
		...
		internal override void DrawDataLabels(ICanvas canvas)
		{
			...
			if (_segments[0] is not PolarLineSegment dataLabel || dataLabel.XPoints == null || dataLabel.YPoints == null)
			{
				return;
			}
		...
		{
			if (CreateSegment() is PolarLineSegment segment)
			{
				segment.Series = this;
