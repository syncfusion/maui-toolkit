...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public partial class AreaSeries : XYDataSeries, IDrawCustomLegendIcon, IMarkerDependent
	{
		...
		internal override void GenerateSegments(SeriesView seriesView)
		{
			...
			for (int i = 0; i < PointsCount; i++)
			{
				...
				if (double.IsNaN(YValues[i]) || i == PointsCount - 1)
				{
					if (xValues != null)
					{
						if (CreateSegment() is AreaSegment segment)
						{
							segment.Series = this;
							...
							{
								if (OldSegments[0] is AreaSegment oldSegment)
								{
		...
		internal override void DrawDataLabels(ICanvas canvas)
		{
			...
			{
				if (segment is not AreaSegment areaSegment || areaSegment?.XValues == null || areaSegment?.YValues == null)
				{
					continue;
				}
		...
		internal override TooltipInfo? GetTooltipInfo(ChartTooltipBehavior tooltipBehavior, float tooltipX, float tooltipY)
		{
			...
			if (ShowMarkers && tooltipInfo != null && _segments.Count > 0)
			{
				if (_segments[0] is IMarkerDependentSegment segment && this is IMarkerDependent series)
				{
