			...
			if (YValues != null)
			{
				...
				for (int i = 0; i < ActualYValues.Count; i++)
				{
					...
					else
					{
						...
						_segments.Add(doughnutSegment);

						if (oldSegments != null)
						{
							if (oldSegments[i] is DoughnutSegment oldSegment)
							{
								doughnutSegment.SetPreviousData([oldSegment.StartAngle, oldSegment.EndAngle]);
							}
						}
					}

					if (_segments[i] is DoughnutSegment segment)
					{
