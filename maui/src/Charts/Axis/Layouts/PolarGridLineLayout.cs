	...
	internal class PolarGridLineLayout : SfDrawableView
    {
        ...
        bool _isRadar
        {
            get
            {
                if (_area != null)
                {
                    return (_area.PolarChart.GridLineType == PolarChartGridLineType.Polygon);
                }
            ...
            if (tickPositions.Count > 0)
            {
                ...
                if (axis.CanDrawMajorGridLines())
                {
                    foreach (var position in tickPositions)
                    {
                        ...
                        {
                            DrawPolarAxisGridLine(canvas, (float)(_desiredSize.Width / 2), (float)(_desiredSize.Height / 2), y, gridLineStyle);
                        }
                        else
                        {
                ...
                if (axis.SmallTickRequired && rangeAxisBase != null && rangeAxisBase.CanDrawMinorGridLines())
                {
                    ...
                    foreach (var pos in smallTicks)
                    {
                        ...
                        if (_area.Series?.Count > 0 && !(_isRadar))
                        {
                            DrawPolarAxisGridLine(canvas, (float)(_desiredSize.Width / 2), (float)(_desiredSize.Height / 2), y, minorGridLineStyle);
                        }
                        else
                        {
        ...
        void DrawPolarAxisGridLine(ICanvas canvas, float centerX, float centerY, float radius, ChartLineStyle gridLineStyle)
        {
        ...
        ChartLineStyle GetDefaultGridLineStyle()
        {
