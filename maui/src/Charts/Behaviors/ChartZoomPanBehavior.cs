...
namespace Syncfusion.Maui.Toolkit.Charts
{
    ...
    public partial class ChartZoomPanBehavior : ChartBehavior
    {
        ...
        bool _isPinchZoomingActivated = false;
...
#if MACCATALYST || IOS
        bool _isScrollChanged = false;
        bool _isSelected = false;
        bool _isPinchZoomed = false;
        Rect _selectedRegionRect;
        DateTime _startTime;
        int _downEntered = 0;
        DateTime _endTime;
#else
        bool _isSelectionZoomingEnded = false;
#endif
        ...
        public void ZoomIn()
        {
            ...
            {
                _cumulativeZoomLevel = _cumulativeZoomLevel + 0.25;
            }
        ...
        bool CanReset(ObservableCollection<ChartAxis> axes)
        {
            if (axes != null)
            {
        ...
        void OnDoubleTap(CartesianChartArea area, Rect clipRect, float pointX, float pointY)
        {
            ...
            _isSelectionZoomingEnded = EnableSelectionZooming ? _isSelectionZoomingEnded : true;
        ...
        float GetOrigin(float manipulationX, float manipulationY, float width, float height, ChartAxis chartAxis)
        {
            float origin;
        ...
        void PinchZoom(ChartAxis chartAxis, Point scaleOrigin, double angle, SizeF size, double scale, bool transpose)
        {
            if (CanZoom(chartAxis, angle, transpose))
            {
                ...
                currentScale *= scale;
                var origin = GetOrigin((float)scaleOrigin.X, (float)scaleOrigin.Y, size.Width, size.Height, chartAxis);
