...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public class ChartTrackballBehavior : ChartBehavior, IMarkerDependent, IThemeElement
    {
        ...
        bool _isAnySideBySideSeries = false;
        bool _isAnyContinuesSeries = false;
        float _currX = 0f;
        Point _linePoint1 = default(Point);
        Point _linePoint2 = default(Point);
        bool _animateMarker = false;
        bool _isScheduled = false;
        float _trackLabelSpacing = 4f;
        List<TrackballAxisInfo> _axisPointInfos;
        ...
        internal bool LongPressActive { get; set; } = false;
        ...
        public ChartTrackballBehavior()
        {
            ...
            _axisPointInfos = new List<TrackballAxisInfo>();
            _previousAxisPointInfos = new List<TrackballAxisInfo>();
            PointInfos = new List<TrackballPointInfo>();
            PreviousPointInfos = new List<TrackballPointInfo>();
            ContentList = new List<SfTooltip>();
        }
        ...
        void UpdateTrackballPointInfos(float pointX, float pointY)
        {
            ...
            if (DisplayMode == LabelDisplayMode.GroupAllPoints)
            {
                if (PointInfos.Count > 0)
                {
                    ...
                    {
                        if (!CartesianChart.TrackballView.Contains(GroupedLabelView))
                        {
                            CartesianChart.TrackballView.Add(GroupedLabelView);
                        }
                    }
        ...
        void GenerateTrackballGroupedLabel()
        {
            if (GroupedLabelView == null)
            {
                ...
                GroupedLabelView.Duration = double.NaN;
                GroupedLabelView.Background = _actualLabelStyle.Background;
            }

            var cellContent = new SfItemViewCell();

            cellContent.ItemTemplate = new DataTemplate(() =>
            {
            ...
            foreach (TrackballPointInfo pointInfo in PointInfos)
            {
                ...
                if (axisPointInfo.HasTrackballAxisTemplate)
                {
                    ...
                    if (trackballTemp == null && ShowLabel)
                    {
                        ...
                        trackballTemp.Duration = double.NaN;
                    ...
                    if (trackballTemp != null)
                    {
                        ...
                        if (axisPointInfo.Label != prevTrackballAxisInfo?.Label)
                        {
                            ...
                            trackballTemp.Content = GetTheTrackballTemplate(axis.TrackballLabelTemplate, axisPointInfo.Label);
                        }
        ...
        void SmartLabelAlignment(SfCartesianChart CartesianChart)
        {
            if (PointInfos.Count > 0)
            {
                ...
                if (CartesianChart.IsTransposed)
                {
                    PointInfos = PointInfos.Where(item => IsRectContainsPoints(item)).OrderBy(item => item.X).ToList();
                }
                else
                {
                    PointInfos = PointInfos.Where(item => IsRectContainsPoints(item)).OrderBy(item => item.Y).ToList();
                }
                //}
            }
            ...
            _intersectedGroups = new List<List<TrackballPointInfo>>();
                ...
                if (intersectedGroupLabels.Count >= 1)
                {
                    for (int j = _intersectedGroups.Count - 1; j >= 0; j--)
                    {
                        ...
                        for (int k = newIntersectedGroups.Count - 1; k >= 0; k--)
                        {
                            ...
                            rect = info.HaveTemplateView && info.ContentTemplateView != null ? info.ContentTemplateView.Helper.TooltipRect : info.TooltipHelper.TooltipRect;
        ...
        {
            if (CartesianChart == null || GroupedLabelView == null) return;

            var plotAreaMargin = CartesianChart.ChartArea.PlotAreaMargin;
            CartesianChart.TrackballView.Padding = plotAreaMargin;
            ...
            if (GroupedLabelView.Content != null)
            {
                ...
                if (CartesianChart.IsTransposed)
                {
                    ...
                    var xPos = SeriesBounds.Width - (contentSize.Width + helper.noseWidth + helper.noseOffset);
                ...
                else
                {
                    ...
                    var yPos = SeriesBounds.Top - _linePoint1.Y + contentSize.Height + helper.noseHeight + helper.noseOffset;
        ...
        void RemoveViews()
        {
            if (GroupedLabelView != null && CartesianChart != null)
            {
                if (CartesianChart.TrackballView.Contains(GroupedLabelView))
                {
                    CartesianChart.TrackballView.Remove(GroupedLabelView);
                }
            }
        }
                    ...
                    if (pointInfo.HaveTemplateView && CartesianChart is SfCartesianChart chart)
                    {
                        ...
                        if (trackballView is not null)
                        {
                            ...
                            if (pointInfo.DataItem != prevTrackballInfo?.DataItem)
                            {
                                ...
                                trackballView.Content = GetTheTrackballTemplate(pointInfo.Series.TrackballLabelTemplate, pointInfo);
                                SetTemplatePosition(chart, trackballView);
                            }
        ...
        SfTooltip? GetOrCreateTrackballView(SfCartesianChart chart, TrackballPointInfo? prevTrackballInfo)
        {
            ...
            if (trackballView == null && ShowLabel)
            {
                ...
                trackballView.Background = _actualLabelStyle.Background;
        ...
        View? GetTheTrackballTemplate(DataTemplate trackballTemplate, object bindingContext)
        {
        ...
        void SetTemplatePosition(SfCartesianChart cartesianChart, SfTooltip trackball)
        {
            if (!cartesianChart.IsTransposed)
            {
                trackball.Helper.PriorityPosition = TooltipPosition.Right;
                trackball.Helper.PriorityPositionList = new List<TooltipPosition>() { TooltipPosition.Left, TooltipPosition.Top, TooltipPosition.Bottom };
            }
            else
            {
                trackball.Helper.PriorityPosition = TooltipPosition.Top;
                trackball.Helper.PriorityPositionList = new List<TooltipPosition>() { TooltipPosition.Bottom, TooltipPosition.Right, TooltipPosition.Left };
            }
        }
        ...
        ChartLabelStyle DefaultAxisLabelStyle(ChartAxis axis)
        {
            return new ChartLabelStyle()
            {
        ...
        void DrawMarker(ICanvas canvas, Rect trackRect, ChartMarkerSettings chartMarkerSettings)
        {
        ...
        string GetAxisLabel(ChartAxis axis, double xValue, string labelFormat)
        {
            ...
            else if (axis is DateTimeAxis datetimeAxis)
            {
                ...
                {
                    format = datetimeAxis.GetSpecificFormattedLabel(datetimeAxis.ActualIntervalType);
                }
    ...
    internal class SfItemViewCell : ContentView
    {
        #region Properties
