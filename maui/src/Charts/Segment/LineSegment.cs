	...
	public partial class LineSegment : CartesianSegment, IMarkerDependentSegment
	{
		...
		protected internal override void Draw(ICanvas canvas)
		{
			...
			if (Series.CanAnimate())
			{
				...
				{
					int index = Series._segments.IndexOf(this);

					if (!Series.XRange.Equals(Series.PreviousXRange) && Series._segments.Count - 1 == index + 1)
					{
						if (Series._segments[index - 1] is LineSegment previousSegment)
						{
