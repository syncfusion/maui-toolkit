...
namespace Syncfusion.Maui.Toolkit.Charts
{
    ...
    public class RangeAreaSegment : AreaSegment, IMarkerDependentSegment
    {
        ...
        void DrawPath(ICanvas canvas, IList<float>? fillPoints, IList<float>? highStrokePoints, IList<float>? lowStrokePoints)
        {
            if (Series == null)
            {
                return;
            ...
            if (fillPoints != null)
            {
                DrawAreaPath(ref _path, fillPoints, isDynamicAnimation, animationValue, PreviousFillPoints);
        ...
        void DrawStroke(ICanvas canvas, float animationValue, bool isDynamicAnimation, IList<float> strokePoints)
        {
            ...
            DrawAreaPath(ref _path, strokePoints, isDynamicAnimation, animationValue, PreviousStrokePoints);
        ...
        void CalculateStrokePoints()
        {
            ...
            {
                var count = HighValues.Length;
                HighStrokePoints = new List<float>();
                LowStrokePoints = new List<float>();
                var start = cartesian.ActualYAxis.VisibleRange.Start;
        ...
        void DrawAreaPath(ref PathF path, IList<float> points, bool isDynamicAnimation, float animationValue, List<float>? previousPoints)
        {
            for (int i = 0; i < points.Count; i++)
            {
                ...
                {
                    x = GetDynamicAnimationValue(animationValue, x, previousPoints[i], x);
                    y = GetDynamicAnimationValue(animationValue, y, previousPoints[i + 1], y);
                }
