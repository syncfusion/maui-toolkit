...
namespace Syncfusion.Maui.Toolkit.Charts
{
    ...
    /// </summary>
    public class SplineRangeAreaSegment : AreaSegment, IMarkerDependentSegment
    {
        #region Fields
        ...
        internal override void OnDataLabelLayout()
        {
            if (Series is SplineRangeAreaSeries series && series.LabelTemplate is not null)
            {
                ...
                if (XVal == null || HighVal == null || LowVal == null || series.ActualData == null) return;
        ...
        void CalculateInteriorPoints()
        {
            ...
            if (series == null || series.ActualXAxis == null || HighVal == null || XVal == null || LowVal == null)
            {
                return;
            }

            var crossingValue = series.ActualXAxis == null ? 0 : series.ActualXAxis.ActualCrossingValue;
            var count = XVal.Length;

            FillPoints = new List<float>();
            HighStartControlPoints = new List<float>();
            HighEndControlPoints = new List<float>();
            LowStartControlPoints = new List<float>();
            LowEndControlPoints = new List<float>();
            StartControlPoints = new List<float>();
            EndControlPoints = new List<float>();

            double highValue = HighVal[0];
        ...
        void DrawPath(ICanvas canvas, IList<float>? fillPoints)
        {
            ...
            {
                halfCount = (StartControlPoints.Count / 2) - 1;
