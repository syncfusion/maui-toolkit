namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public partial class PieSegment : ChartSegment
	{
		...
		const float LabelGap = 5;

		internal double _connectorLength = 0.1;

		internal float _connectorBendLineLength = 15;
		...
		internal double _isLabelUpdated;
		...
		{
			if (double.IsNaN(YValue) || Series is not PieSeries pieSeries)
			{
				return;
			}
			...
			MidAngle = pieSeries._isClockWise ? (StartAngle + (EndAngle / 2)) * 0.0174532925f : (StartAngle + EndAngle) / 2 * 0.0174532925f;
		...
		protected internal override void Draw(ICanvas canvas)
		{
			...
			var endArcAngle = pieSeries._isClockWise ? drawStartAngle + drawEndAngle : drawEndAngle;

			//Drawing segment.
			var path = new PathF();
			path.MoveTo(_currentBounds.Left + (_currentBounds.Width / 2), _currentBounds.Top + (_currentBounds.Height / 2));
			path.AddArc(_currentBounds.Left, _currentBounds.Top, _currentBounds.Right, _currentBounds.Bottom, -drawStartAngle, -endArcAngle, pieSeries._isClockWise);
			path.Close();
		...
		internal virtual void SetData(double startAngle, double endAngle, double yValue)
		{
			if (Series is PieSeries pieSeries)
			{
				...
				EndAngle = pieSeries._isClockWise ? endAngle : startAngle + endAngle;
				YValue = yValue;
			}
		}
		...
		internal static float GetDynamicAnimationAngleValue(float animationValue, double oldAngle, double currentAngle)
		{
		...
		internal override int GetDataPointIndex(float x, float y)
		{
			...
			{
				return pieSeries._segments.IndexOf(this);
			}
		...
		internal override void OnDataLabelLayout()
		{
			if (Series is not PieSeries pieSeries || double.IsNaN(YValue) || pieSeries == null
				|| pieSeries.DataLabelSettings == null)
			{
				return;
			}
		...
		bool IsPointInPieSegment(double radius, double x, double y)
		{
			if (Series is PieSeries pieSeries)
			{
				...
				double endAngle = pieSeries._isClockWise ? Math.Abs(StartAngle) + Math.Abs(EndAngle) : 360 + EndAngle;
				...
				if (pointLength <= radius)
				{
					if (pieSeries.StartAngle > pieSeries.EndAngle)
					{
						if (pieSeries._isClockWise && StartAngle > angle && angle > endAngle)
						{
							return true;
						}
						else if (!pieSeries._isClockWise && startAngle > angle && angle > endAngle)
						{
							return true;
						}
					}
		...
		PointF GetDataLabelPosition(CircularSeries series, PieSegment dataLabel, CircularDataLabelSettings dataLabelSettings, SizeF labelSize, PointF labelPosition, float padding)
		{
			...
			if (dataLabelSettings.LabelPosition == ChartDataLabelPosition.Inside)
			{
				...
				PointF point1 = GetPositionForInsideSmartLabel(series, dataLabel, x1, y1, labelSize, borderWidth, padding, labelStyle);
				x1 = point1.X;
				y1 = point1.Y;
			}
			else
			{
				PointF point = GetPositionForOutsideSmartLabel(series, dataLabel, labelSize, borderWidth, padding, labelStyle);
				x1 = point.X;
				y1 = point.Y;
			}
		...
		PointF GetPositionForInsideSmartLabel(CircularSeries series, PieSegment? dataMarkerLabel, float actualX, float actualY, SizeF labelSize, float borderWidth, float padding, ChartDataLabelStyle style)
		{
			if (dataMarkerLabel == null)
			{
				return PointF.Zero;
			}
			...
			if (series.DataLabelSettings.SmartLabelAlignment == SmartLabelAlignment.Shift)
			{
				...
				{
					if (series._segments[i] is not PieSegment pieSegment)
					{
						return PointF.Zero;
					}

					if (CircularSeries.IsOverlap(dataMarkerLabel.LabelRect, pieSegment.LabelRect))
					{
						isIntersected = true;
					}
				}
			...
			if (isIntersected)
			{
				...
				Point rect = GetPositionForOutsideSmartLabel(series, dataMarkerLabel, labelSize, borderWidth, padding, style);
			...
			double offsetX = (series.DataLabelSettings.SmartLabelAlignment == SmartLabelAlignment.None)
			   ? (double.IsNaN(style.OffsetX) ? 0 : style.OffsetX) : 0;

			double offsetY = (series.DataLabelSettings.SmartLabelAlignment == SmartLabelAlignment.None)
							? (double.IsNaN(style.OffsetY) ? 0 : style.OffsetY) : 0;

			_innerRect.X = point2.X + (float)offsetX;
			_innerRect.Y = point2.Y + (float)offsetY;
...
#pragma warning disable IDE0060 // Remove unused parameter
		PointF GetPositionForOutsideSmartLabel(CircularSeries series, PieSegment dataMarkerLabel, SizeF labelSize, float borderWidth, float padding, ChartDataLabelStyle style)
#pragma warning restore IDE0060 // Remove unused parameter
#pragma warning restore IDE0060 // Remove unused parameter
		{
			if (series is not PieSeries pieSeries || series.AreaBounds == Rect.Zero)
			{
				return PointF.Zero;
			}
			...
			PointF center = series.GetCenter();
			double dataLabelMidRadius = radius + (radius * _connectorLength);
			PointF startPoint = CircularSeries.calculateOffset(
				   SegmentMidAngle!, radius, center);
			PointF endPoint = CircularSeries.calculateOffset(SegmentMidAngle!,
			(dataLabelMidRadius), center);

			dataMarkerLabel.XPoints = [];
			dataMarkerLabel.YPoints = [];
			...
			{
				XPoints[2] = endPoint.X + _connectorBendLineLength + (float)offsetX;
				YPoints[2] = endPoint.Y + (float)offsetY;
				rect = new Rect(XPoints[2] + LabelGap + labelSize.Width / 2, YPoints[2], labelSize.Width, labelSize.Height);
			}
			else
			{
				XPoints[2] = endPoint.X - _connectorBendLineLength - (float)offsetX;
				YPoints[2] = endPoint.Y + (float)offsetY;
				rect = new Rect(XPoints[2] - labelSize.Width / 2 - LabelGap, YPoints[2], labelSize.Width, labelSize.Height);
			}
