    ...
    public class RangeColumnSegment : ColumnSegment
    {
        ...
        void LayoutSegment()
        {
            ...
            if (series.CanAnimate())
            {
                ...
                {
                    y1Value = GetColumnDynamicAnimationValue(animationValue, PreviousY1, _y1);
                    y2Value = GetColumnDynamicAnimationValue(animationValue, PreviousY2, _y2);
                }
            }
            ...
            for (int i = 0; i < 2; i++)
            {
                if (Series is RangeColumnSeries series)
                {
                    ...
                    if (DataLabels != null && DataLabels.Count > i)
                    {
                        ...
                        if (i == 0)
                        {
                            LabelPositionPoints[i] = series.IsDataInVisibleRange(_xValue, _y1) && !Empty ? dataLabelSettings.CalculateDataLabelPoint(series, this, highPoint, dataLabelSettings.LabelStyle, "HighType") : new PointF(float.NaN, float.NaN);

                        }
                        else if (i == 1)
                        {
                            LabelPositionPoints[i] = series.IsDataInVisibleRange(_xValue, _y2) && !Empty ? dataLabelSettings.CalculateDataLabelPoint(series, this, lowPoint, dataLabelSettings.LabelStyle, "LowType") : new PointF(float.NaN, float.NaN);
                        }
        ...
        float GetColumnDynamicAnimationValue(float animationValue, double oldValue, double currentValue)
        {
