...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	/// </summary>
	public partial class PolarLineSegment : ChartSegment, IMarkerDependentSegment
	{
		#region Fields
		...
		internal override int GetDataPointIndex(float x, float y)
		{
			if (Series != null)
			{
				...
				else if (Series._segments.IndexOf(this) == Series._segments.Count - 1 && IsRectContains((float)_x2Value, (float)_y2Value, x, y, (float)StrokeWidth))
				{
		...
		{
			if (Series is not PolarSeries polarSeries)
			{
				return;
			}
			...
			if (polarSeries.ChartArea == null || !polarSeries.ShowDataLabels || dataLabelSettings == null)
			{
				return;
			}
		...
		{
			if (Series is not PolarSeries series || SeriesView == null || Empty)
			{
				return;
			}
