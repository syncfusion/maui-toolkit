namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	/// </summary>
	public partial class RadialBarSegment : ChartSegment
	{
		#region Fields
		...
		{
			if (_currentSeries == null || _currentSeries.YValues[Index] == 0)
			{
				return;
			}
		...
		void SegmentEndCurveCalculation(float midRadius, float segmentRadius)
		{
			...
			if (_currentSeries.StartAngle < _currentSeries.EndAngle)
			{
				...
				_segmentEndAngle -= (2 * _angleDeviation);
		...
		{
			if (_currentSeries == null || segmentRadius == 0)
			{
				return;
			}
		...
		{
			if (Series is RadialBarSeries radialBarSeries)
			{
				double dx, dy;
		...
		{
			if (_currentSeries == null || (_currentSeries.YValues[Index] == 0 && !_isTrack))
			{
				return;
			}
		...
		void CalculateSegmentRadius()
		{
			...
			minScale *= _currentSeries.Radius;
			...
			double radius = (_actualRadius / _currentSeries._visibleSegmentCount) * (1 - _currentSeries.GapRatio);
			InnerRingRadius = _innerRadius + (_actualRadius / _currentSeries._visibleSegmentCount) * VisibleSegmentIndex + centerRadius;
