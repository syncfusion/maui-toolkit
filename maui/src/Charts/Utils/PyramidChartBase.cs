...
namespace Syncfusion.Maui.Toolkit.Charts
{
    ...
    {
        const int _spacing = 3;
        const float _bendRatio = 0.05f;
        static float _desiredWidth = float.MinValue;
        readonly IPyramidChartDependent _chart;
        internal readonly ObservableCollection<IPyramidLabels> Segments;
        Dictionary<IPyramidLabels, RectF> _labelRects;
        ...
        public PyramidDataLabelHelper(IPyramidChartDependent chart)
        {
            ...
            _labelRects = new Dictionary<IPyramidLabels, RectF>();
        }
        ...
        internal void ArrangeElements()
        {
            ...
            _desiredWidth = (float)(clip.Width - (seriesBounds.X + seriesBounds.Width));
            _labelRects.Clear();
        ...
        Rect CalculateLabelRect(IPyramidLabels item, DataLabelPlacement placement, Size size)
        {
            ...
            switch (placement)
            {
                ...
                case DataLabelPlacement.Outer:
                    ...
                    var bend = (outerX - bounds.Center.X) * _bendRatio;
                    ...
                    var width = size.Width < _desiredWidth ? size.Width : _desiredWidth;
        ...
        Rect ArrangeSmartLabel(IPyramidLabels item, DataLabelPlacement actualPosition, Rect labelRect)
        {
            foreach (var rect in _labelRects)
            {
                ...
                else if (IsIntersected && actualPosition == DataLabelPlacement.Outer)
                {
                    ...
                    item.IsLabelVisible = true;
                    item.DataLabelY = !_chart.ArrangeReverse ? adjacentRect.Y + adjacentRect.Height + _spacing + (float)labelRect.Height / 2
                        : adjacentRect.Y - _spacing - (float)labelRect.Height / 2;
                    labelRect = CalculateLabelRect(item, actualPosition, labelRect.Size);
                }
                else
                {
        ...
        void DrawLabel(ICanvas canvas, IPyramidLabels item, Rect rect, ChartDataLabelStyle style)
        {
#if ANDROID
        ...
        void DrawBackground(ICanvas canvas, Brush fill, ChartDataLabelStyle style, Rect backgroundRect)
        {
            canvas.CanvasSaveState();
        ...
        void DrawLabel(ICanvas canvas, string label, PointF point, ChartDataLabelStyle style)
        {
