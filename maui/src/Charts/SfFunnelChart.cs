	...
	public class SfFunnelChart : ChartBase, IPyramidChartDependent, IDrawCustomLegendIcon, ITapGestureListener, ITouchListener, IParentThemeElement
    {
        ...
        internal override TooltipInfo? GetTooltipInfo(ChartTooltipBehavior behavior, float x, float y)
        {
            if (EnableTooltip)
            {
                ...
                {
                    X = segment.SegmentBounds.Center.X + (float)seriesBounds.Left,
                    Y = segment.SegmentBounds.Center.Y + (float)seriesBounds.Top,
                    Index = index,
        ...
        void CalculateSegment(double total, IList<double> yValues)
        {
            if (yValues != null)
            {
                ...
                for (int i = _pointsCount - 1; i >= 0; i--)
                {
                    ...
                    if (_pointsCount <= _segments.Count && _segments[i] is FunnelSegment)
                    {
                        ((FunnelSegment)_segments[i]).SetData(currY, height, NeckWidth, NeckHeight, GetActualXValue(i), yValue);
                    }
                    else
                    {
        ...
        {
            if (sender is not ChartDataLabelSettings dataLabelSettings) return;
