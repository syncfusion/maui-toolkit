...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public partial class SfPyramidChart : ChartBase, IPyramidChartDependent, IDrawCustomLegendIcon, ITapGestureListener, ITouchListener, IParentThemeElement
	{
		...
		public static readonly BindableProperty PointColorPathProperty =
			BindableProperty.Create(nameof(PointColorPath), typeof(string), typeof(SfPyramidChart), null, BindingMode.Default, null, propertyChanged: OnPointColorPathChanged);
		...
		/// </value>
		/// <remarks>
		/// The <see cref="Orientation"/> property allows the pyramid chart to be displayed either vertically (top to bottom) or horizontally (left to right).
		/// </remarks>
		/// <example>
		...
		static void OnPointColorPathChanged(BindableObject bindable, object oldValue, object newValue)
		{
			...
			if (chart != null)
			{
				...
				{
					chart.colorComplexPaths = ((string)newValue).Split(new char[] { '.' });
				}
		...
		{
			if (bindable is SfPyramidChart chart && oldValue != newValue)
			{
...
#pragma warning restore IDE0060 // Remove unused parameter
		...
		private void SetIndividualPoint(int index, object value, bool replace)
		{
			...
			if (!string.IsNullOrEmpty(PointColorPath))
			{
				setColorValue = () =>
				{
					var colorValue = GetArrayPropertyValue(value, colorComplexPaths);
