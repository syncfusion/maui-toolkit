...
namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public class SfPyramidChart : ChartBase, IPyramidChartDependent, IDrawCustomLegendIcon, ITapGestureListener, ITouchListener, IParentThemeElement
    {
        ...
        public SfPyramidChart()
        {
            ...
            _chartArea = (PyramidChartArea)LegendLayout.AreaBase;
            _yValues = new List<double>();
            _segments = new ObservableCollection<ChartSegment>();
        ...
        void IPyramidChartDependent.UpdateLegendItemsSource(ObservableCollection<ILegendItem> legendItems)
        {
            ...
            for (int i = 0; i < _pointsCount; i++)
            {
                ...
                ((IChartPlotArea)this._chartArea).UpdateLegendLabelStyle(legendItem, Legend.LabelStyle ?? LegendStyle);
        ...
        static void OnXBindingPathChanged(BindableObject bindable, object oldValue, object newValue)
        {
            if (bindable is SfPyramidChart chart)
            {
                ...
                {
                    chart._xComplexPaths = ((string)newValue).Split(new char[] { '.' });
                }
        ...
        void CalculateLinearSegment(double total, double gapRatio, IList<double> yValues)
        {
            if (yValues != null)
            {
                ...
                for (int i = 0; i < _pointsCount; i++)
                {
                    ...
                    if (i < _segments.Count && _segments[i] is PyramidSegment)
                    {
                        ((PyramidSegment)_segments[i]).SetData(currentY, height, GetActualXValue(i), yValue);
        ...
        void CalculateSurfaceSegment(double total, double gapHeight, IList<double> yValues)
        {
            ...
            for (int i = 0; i < _pointsCount; i++)
            {
                ...
                if (i < _segments.Count && _segments[i] is PyramidSegment)
                {
                  ...
                  ((PyramidSegment)_segments[i]).SetData(currentHeight, coef * height[i], GetActualXValue(i), yValue);
                }
                else
                {
                    if (!double.IsNaN(yValues[i]))
        ...
        {
            ResetData();
            GeneratePoints(new[] { YBindingPath }, _yValues);
            UpdateLegendItems();
        ...
        {
            if (_xValues is IList<double>)
            {
                ((IList<double>)_xValues).RemoveAt(index);
                _pointsCount--;
            }
            else if (_xValues is IList<string>)
            {
                ((IList<string>)_xValues).RemoveAt(index);
                _pointsCount--;
            }
        ...
        ChartValueType GetDataType(IEnumerator enumerator, string[] paths)
        {
        ...
        void ResetData()
        {
            if (_actualXValues is IList && _xValues is IList)
            {
                ((IList)_xValues).Clear();
                ((IList)_actualXValues).Clear();
            }
            ...
            if (XBindingPath != null && _yPaths != null && _yPaths.Any())
            {
                _segments.Clear();
            }
        }

        ChartValueType GetDataType(FastReflection fastReflection, IEnumerable dataSource)
        {
            if (dataSource == null)
            {
        ...
        ChartValueType GetDataType(object? xValue)
        {
        ...
        void GeneratePoints(string[] yBindingPaths, params IList<double>[] yValueLists)
        {
            ...
            for (int i = 0; i < yBindingPaths.Length; i++)
            {
                ...
                _yComplexPaths[i] = yBindingPaths[i].Split(new char[] { '.' });
