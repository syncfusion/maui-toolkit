...
namespace Syncfusion.Maui.Toolkit.Charts
{
    internal partial class CartesianChartArea
    {
        ...
        internal void UpdateStackingSeries()
        {
            ...
            _seriesGroup = new Dictionary<string, List<StackingSeriesBase>>();
            ...
            foreach (var series in VisibleSeries)
            {
                if (series is StackingSeriesBase stackingSeries && stackingSeries.IsVisible)
                {
                    ...
                    if (_seriesGroup.ContainsKey(stackingGroup))
                    {
                        var seriesList = _seriesGroup[stackingGroup];
                        if (seriesList.Any(x => x.ActualXAxis != stackingXAxis || x.ActualYAxis != stackingYAxis) || (seriesList[0].GetType() != stackingSeries.GetType() && stackingGroup != stackingSeries.GroupingLabel))
                        {
                            ...
                            seriesList = key != null ? _seriesGroup[key] : new List<StackingSeriesBase>();
                        }
                        ...
                        {
                            _seriesGroup.Add(stackingGroup + _seriesGroup.Count, new List<StackingSeriesBase>() { stackingSeries });
                        }
                    }
                    else
                    {
                        _seriesGroup.Add(stackingGroup, new List<StackingSeriesBase>() { stackingSeries });
                    }
                }
            }
