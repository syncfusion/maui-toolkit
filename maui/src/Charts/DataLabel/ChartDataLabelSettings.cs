namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public abstract class ChartDataLabelSettings : BindableObject
	{
		...
		internal List<string> IsNeedDataLabelMeasure { get; set; } = [nameof(LabelPlacement), nameof(LabelStyle)];
		...
		internal Color GetContrastTextColor(ChartSeries series, Brush? background, Brush? segmentBrush)
		{
			...
			else
			{
				...
				{
					textColor = series.IsIndividualSegment() ? segmentBrush is SolidColorBrush ? ChartUtils.GetContrastColor((segmentBrush as SolidColorBrush)?.Color) : textColor : ChartDataLabelSettings.GetTextColorBasedOnChartBackground(series?.Chart);
				}
				else if (LabelPlacement == DataLabelPlacement.Outer)
				{
					textColor = ChartDataLabelSettings.GetTextColorBasedOnChartBackground(series?.Chart);
				}
			}
		...
		internal static PointF GetAutoLabelPosition(ChartSeries chartSeries, float x, float y, SizeF labelSize, float padding, float borderWidth)
		{
			if (chartSeries == null)
			{
				return PointF.Zero;
			...
			Size size = chartSeries is ScatterSeries scatter ? new Size(scatter.PointWidth, scatter.PointHeight) : Size.Zero;
		...
		static Color GetTextColorBasedOnChartBackground(IChart? chart)
		{
