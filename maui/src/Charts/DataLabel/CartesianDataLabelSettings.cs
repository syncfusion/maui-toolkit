namespace Syncfusion.Maui.Toolkit.Charts
{
	...
	public partial class CartesianDataLabelSettings : ChartDataLabelSettings
	{
		...
		internal static PointF CalculateDataLabelPoint(XYDataSeries xyDataSeries, ChartSegment dataLabel, PointF labelPoint, ChartDataLabelStyle labelStyle)
		{
		...
		{
			if (xyDataSeries.ActualYAxis == null)
			{
				return labelPoint;
			}
			...
			if (LabelPlacement == DataLabelPlacement.Inner || LabelPlacement == DataLabelPlacement.Auto)
			{
				...
				if (LabelPlacement == DataLabelPlacement.Auto)
				{
					...
					PointF point = ChartDataLabelSettings.GetAutoLabelPosition(xyDataSeries, x, y, labelSize, padding, borderWidth);
					x = point.X;
					y = point.Y;
				}
		...
		{
			bool isTop = CartesianDataLabelSettings.IsTopWithLabelIndex(xyDataSeries, index);
		...
		{
			if (xyDataSeries.ChartArea == null)
			{
				return labelPoint;
			}
		...
		internal PointF CalculateDataLabelPoint(RangeSeriesBase series, RangeColumnSegment dataLabel, PointF labelPoint, ChartDataLabelStyle labelStyle, string valueType)
		{
			...
			SizeF labelSize = series.LabelTemplate is null ? labelStyle.MeasureLabel(dataLabel._dataLabel[0] ?? string.Empty) : series.GetLabelTemplateSize(dataLabel, valueType);
			float padding = (float)labelStyle.LabelPadding;
		...
		static bool IsTopWithLabelIndex(XYDataSeries xyDataSeries, int index)
		{
				...
				case "HighType":
					if (series.ChartArea != null && series.ChartArea.IsTransposed)
					{
						if (LabelPlacement == DataLabelPlacement.Auto || LabelPlacement == DataLabelPlacement.Inner)
						{
							...
							PointF point = ChartDataLabelSettings.GetAutoLabelPosition(series, x, y, labelSize, padding, borderWidth);
							x = point.X;
							y = point.Y;
						}
						else if (LabelPlacement == DataLabelPlacement.Outer)
		...
		PointF GetLabelPositionForCandleSeries(FinancialSeriesBase candleSeries, int index, SizeF labelSize, PointF labelPoint, float padding, string valueType)
		{
			...
			if (LabelPlacement == DataLabelPlacement.Auto)
			{
				PointF point = ChartDataLabelSettings.GetAutoLabelPosition(candleSeries, x, y, labelSize, padding, borderWidth);
				x = point.X;
				y = point.Y;
			}
