...
namespace Syncfusion.Maui.Toolkit.SegmentedControl
{
	...
	/// </summary>
#if WINDOWS
    internal class SegmentLayout : SfView, IKeyboardListener
#else
...
#endif
    {
        ...
        /// </summary>
        private double _segmentLayoutHeight = 0;
#endif
        ...
        internal void UpdateItemSelection()
        {
            ...
            SegmentItemView? olditemView = Children.OfType<SegmentItemView>().FirstOrDefault(x => x.IsSelected);
        ...
        internal void TriggerSelectionChanged()
        {
            ...
            newItemView.UpdateSelection();
            SelectionChangedEventArgs eventArgs = new SelectionChangedEventArgs() { OldIndex = -1, NewIndex = newIndex, OldValue = null, NewValue = newItemView.SegmentItem };
        ...
        internal void RemoveSegmentItemView(int index)
        {
            ...
            {
                child.Dispose();
                UnWireSegmentItemPropertyChanged(child.SegmentItem);
                Remove(child);
        ...
        internal void AddSegmentItemView(int index)
        {
            ...
            if (_segmentItemViews == null)
            {
                _segmentItemViews = new List<SegmentItemView>();
            }
        ...
        void OnSegmentItemPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            {
                foreach (SegmentItemView child in Children)
                {
                    if (child.SegmentItem.Equals(segmentItem))
                    {
        ...
        protected override Size MeasureContent(double widthConstraint, double heightConstraint)
        {
            ...
            if (!double.IsFinite(widthConstraint))
            {
                for (int i = 0; i < Children.Count; i++)
                {
                    ...
                    double itemWidth = SegmentItemViewHelper.GetItemWidth(_itemInfo, child.SegmentItem);
                    if (showSeparator)
                    {
        ...
        protected override Size ArrangeContent(Rect bounds)
        {
            ...
            {
                SegmentItemView child = (SegmentItemView)Children[i];
                width = SegmentItemViewHelper.GetItemWidth(_itemInfo, child.SegmentItem);
                if (showSeparator)
                {
