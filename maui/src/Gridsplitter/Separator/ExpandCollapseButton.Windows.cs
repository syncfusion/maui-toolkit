...
{
    /// <summary>
    /// Windows-specific partial of <see cref="ExpandCollapseButton"/>.
    /// Implements the touch path on Windows. The button has no
    /// platform-specific behavior beyond the cross-platform press /
    /// release / click handling, so the implementation is shared with
    /// iOS, Android, and MacCatalyst.
    /// </summary>
    internal sealed partial class ExpandCollapseButton
    {
        /// <summary>
        /// Windows implementation of <see cref="ITouchListener.OnTouch"/>.
        /// Tracks the pressed state, raises <see cref="Clicked"/> on
        /// release while still pressed, and re-renders the border
        /// thickness on Entered/Exited.
        /// </summary>
