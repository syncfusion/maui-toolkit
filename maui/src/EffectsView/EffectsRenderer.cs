...
namespace Syncfusion.Maui.Toolkit.EffectsView
{
	...
	internal class EffectsRenderer : ITouchListener
	{
		...
		/// </summary>
		internal ObservableCollection<RectF> RippleBoundsCollection { get; set; } = [];

		/// <summary>
		/// Gets or sets the highlight bounds collection value.
		/// </summary>
		internal ObservableCollection<RectF> HighlightBoundsCollection { get; set; } = [];
		...
		/// </summary>
		internal double ControlWidth { get; set; }
		...
		internal void DrawEffects(ICanvas canvas, bool isCornerRadius = true)
		{
			if (HighlightBounds.Width > 0 && HighlightBounds.Height > 0)
			{
				...
				_currentCanvas = isCornerRadius ? EffectsRenderer.GetCornerRadiusCanvas(canvas, HighlightBounds) : canvas;
			...
			{
				canvas.CanvasSaveState();
				_currentCanvas = isCornerRadius ? EffectsRenderer.GetCornerRadiusCanvas(canvas, RippleBounds) : canvas;
		...
		/// <returns></returns>
		static ICanvas GetCornerRadiusCanvas(ICanvas canvas, RectF bounds)
		{
