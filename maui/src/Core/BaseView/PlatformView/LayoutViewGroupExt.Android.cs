...
namespace Syncfusion.Maui.Toolkit.Platform
{
	internal class LayoutViewGroupExt : LayoutViewGroup, AccessibilityManager.IAccessibilityStateChangeListener
	{
		...
		/// </summary>
		private CustomAccessibilityDelegate? _customAccessibilityDelegate;
		...
		internal DrawingOrder DrawingOrder
		{
			...
			set
			{
				...
				_customAccessibilityDelegate?.UpdateChildOrder(_drawingOrder != DrawingOrder.BelowContent);
			}
		}
		...
		internal void InvalidateSemantics()
		{
			...
			_customAccessibilityDelegate?.InvalidateSemantics();
		}
		...
		protected override bool DispatchHoverEvent(MotionEvent? e)
		{
			...
			{
				AccessibilityManager? accessibility = (AccessibilityManager?)Context.GetSystemService(Context.AccessibilityService!);
				if (accessibility != null && accessibility.IsEnabled && _customAccessibilityDelegate != null && _customAccessibilityDelegate.DispatchHoverEvent(e))
				{
					return true;
				}
			}
		...
		{
			if (!enabled || (_customAccessibilityDelegate != null && AndroidX.Core.View.ViewCompat.GetAccessibilityDelegate(this) == _customAccessibilityDelegate))
			{
				return;
			}
			...
			_customAccessibilityDelegate = new CustomAccessibilityDelegate(this, drawableView, DrawingOrder != DrawingOrder.BelowContent);
			AndroidX.Core.View.ViewCompat.SetAccessibilityDelegate(this, _customAccessibilityDelegate);
		}
		...
		private void AddAccessibility(Context context, Microsoft.Maui.Controls.View view)
		{
			...
			_customAccessibilityDelegate = new CustomAccessibilityDelegate(this, view, DrawingOrder != DrawingOrder.BelowContent);
			AndroidX.Core.View.ViewCompat.SetAccessibilityDelegate(this, _customAccessibilityDelegate);
		}
		...
		public override bool OnInterceptTouchEvent(MotionEvent? ev)
		{
			...
			if (Drawable is PullToRefreshBase pullToRefresh && pullToRefresh.OnInterceptTouchEvent(ev))
			{
				return true;
			}

			if (Drawable is SfNavigationDrawerExt navigationDrawer && navigationDrawer.OnInterceptTouchEvent(ev))
			{
				return true;
			}

			if (Drawable is SfTabViewExt tabView && tabView.OnInterceptTouchEvent(ev))
			{
				return true;
			}
