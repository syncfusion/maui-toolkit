...
namespace Syncfusion.Maui.Toolkit.Carousel
{
	...
	public partial class PlatformCarousel : Selector, IDisposable
    {
        ...
        /// </summary>
        internal IEnumerable? TempCollection;

        /// <summary>
        /// Disable with out animation scroll
        /// </summary>
        internal bool IsDisableScrollChange = false;

        /// <summary>
        /// Update intermediate scroll
        /// </summary>
        internal bool IsIntermediate = false;

        /// <summary>
        /// Maintain collection for UIVirtualization
        /// </summary>
        internal ObservableCollection<object>? TabCollection;

        /// <summary>
        /// Maintain collection for virtualized UI elements.
        /// </summary>
        internal ObservableCollection<object> VirtualItemList = new();

        /// <summary>
        /// Declaration for CDesiredDeceleration 
        /// </summary>
        private const double _CDesiredDeceleration = 10.0 * 96.0 / (1000.0 * 1000.0);
        ...
        /// </summary>
        private object? _internalSelectedItem = null;
        ...
        /// </summary>
        private bool _isSelectionNotChange = false;
        ...
        {
            var tempCollection = TempCollection as IList;
            if (tempCollection != null && ((SelectedIndex < tempCollection.Count - 1 && isForward) || (SelectedIndex > 0 && !isForward)))
            {
                int newIndex = SelectedIndex + (isForward ? 1 : -1);
                var newItem = VirtualItemList[newIndex] as PlatformCarouselItem;
        ...
        {
            if (LoadMoreItemsCount > 0 && Queue.Count > 0)
            {
                UpdateSource();
            }
        }
		...
		public void MovePrevious()
        {
            ...
            {
                var tempCollection = TempCollection as IList;
                if (tempCollection != null && ((SelectedIndex > 0 && !isBackward) || (SelectedIndex < tempCollection.Count - 1 && isBackward)))
                {
        ...
        /// <summary>
        /// Validate Selected Index to move previous item
        /// </summary>
        /// <param name="flowFactor"></param>
        void ValidateIndexToMovePrevious(bool flowFactor)
        {
            ...
            else if (SelectedIndex < Items.Count - 1 && !flowFactor)
            {
                SelectedIndex++;
        ...
        internal void Refresh(PlatformCarouselItem item)
        {
            ...
            {
                if (ViewMode == ViewMode.Default && EnableVirtualization && VirtualItemList != null)
                {
                    int index = ((IList)VirtualItemList).IndexOf(item);
        ...
        {
            if (_carouselItemsPanel == null) return;
        ...
        int GetItemIndex(PlatformCarouselItem? item)
        {
            ...
            else
            {
                if (item != null)
                {
                    return VirtualItemList.IndexOf(item);
                }
            }
        ...
        internal void UpdateLinearPanel()
        {
            ...
            {
                if (_carouselLinearPanel == null) return;
        ...
        int CalculateZIndex(int selectedIndex)
        {
            ...
            if (!AllowLoadMore)
            {
                ...
                else if(TempCollection != null)
                {
                    zIndex = ((IList)TempCollection).Count - Math.Abs(selectedIndex);
                }
            }
        ...
        internal void UpdateSource()
        {
            if (Queue.Count != 0)
            {
                if (Queue.Count < LoadMoreItemsCount)
                {
                    DequeueItems(Queue.Count);
                }
                else
                {
        ...
        void DequeueItems(int count)
        {
            ...
            {
                ((IList)base.ItemsSource).Add(Queue.Dequeue());
            }
            UpdateLinearPanel();
        }
        ...
        {
            if (Queue.Count <= 0 && AllowLoadMore)
            {
        ...
        void RemoveLoadMoreViewFromParent()
        {
            if (ViewMode == ViewMode.Default)
            {
                if (CarouselItemsPanel != null && CarouselItemsPanel.Children.Count  > 0 && ((PlatformCarouselItem)CarouselItemsPanel.Children[(CarouselItemsPanel.Children.Count - 1)]).Tag != null && ((PlatformCarouselItem)CarouselItemsPanel.Children[(CarouselItemsPanel.Children.Count - 1)]).Tag.ToString() == "Load More")
                {
                    ((PlatformCarouselItem)CarouselItemsPanel.Children[(CarouselItemsPanel.Children.Count - 1)]).Content = null;
                    CarouselItemsPanel.Children.RemoveAt(CarouselItemsPanel.Children.Count - 1);
                }
            }
            else
            {
                if (CarouselLinearPanel != null && CarouselLinearPanel.Children.Count > 0 && ((PlatformCarouselItem)CarouselLinearPanel.Children[(CarouselLinearPanel.Children.Count - 1)]).Tag != null && ((PlatformCarouselItem)CarouselLinearPanel.Children[(CarouselLinearPanel.Children.Count - 1)]).Tag.ToString() == "Load More")
                {
                    ((PlatformCarouselItem)CarouselLinearPanel.Children[(CarouselLinearPanel.Children.Count - 1)]).Content = null;
        ...
        internal void UpdateItems()
        {
            if ((_start != -1 && _end != -1 && _preStart == -1 && _preEnd == -1) || _preEnd < _start)
            {
                ...
                {
                    TabCollection?.Clear();
                    base.ItemsSource = TabCollection;
        ...
        void RemoveItemsFromEnd(int count)
        {
            for (int i = 0; i < count; i++)
            {
                TabCollection?.RemoveAt(TabCollection.Count - 1);
            }
        }
		...
		protected override void PrepareContainerForItemOverride(DependencyObject element, object item)
        {
            ...
            if (carouselItem != null)
            {
                ...
                binding.Path = new PropertyPath("SelectedItemTemplate");
                binding.Source = this;
                ...
                if (ItemLoaded != null)
                {
                    ...
                    {
                        arg.Index = ((IList)TempCollection).IndexOf(carouselItem);
                    }
		...
		protected override void OnKeyDown(KeyRoutedEventArgs e)
        {
            ...
            else if (e.Key == Windows.System.VirtualKey.PageDown || e.Key == Windows.System.VirtualKey.End)
            {
                ...
                if (virtualView != null)
                    virtualView.SelectedIndex = Items.Count - 1; ;
            }
        ...
        {
            e.TranslationBehavior.DesiredDeceleration = _CDesiredDeceleration;
        ...
        {
            var tempCollection = TempCollection as IList;
            if (EnableVirtualization && tempCollection != null && SelectedIndex < tempCollection.Count)
            {
		...
		protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (this != null)
            {
                Microsoft.Maui.Platform.WrapperView? parentElement = this.Parent as Microsoft.Maui.Platform.WrapperView;
                if (parentElement != null && parentElement.FlowDirection != this.FlowDirection)
                    parentElement.FlowDirection = this.FlowDirection;
            }
        ...
        void HandleHorizontalScroll()
        {
            if (_scrollViewer != null && _scrollViewer.HorizontalOffset == _scrollViewer.ExtentWidth - _scrollViewer.ViewportWidth)
            {
                ...
                {
                    if (ScrollViewer != null && ScrollViewer.HorizontalOffset > _preOffset && _end != ((IList)TempCollection).Count - 1)
                    {
                        AddForward(false);
                    }
                }
        ...
        void HandleVerticalScroll()
        {
            if (_scrollViewer != null && _scrollViewer.VerticalOffset == _scrollViewer.ExtentHeight - _scrollViewer.ViewportHeight)
            {
                ...
                IsIntermediate = true;
                if (ScrollViewer != null && ScrollViewer.VerticalOffset > _preOffset && TempCollection != null && _end != ((IList)TempCollection).Count - 1)
                {
                    AddForward(false);
                }
            }
        ...
        void RemoveOldView()
        {
            if (AllowLoadMore)
            {
                if (CarouselItemsPanel != null)
                {
                    if ((CarouselItemsPanel.Children.Count > 0 && (((PlatformCarouselItem)CarouselItemsPanel.Children[CarouselItemsPanel.Children.Count - 1]).Tag != null))
                        && (((PlatformCarouselItem)CarouselItemsPanel.Children[CarouselItemsPanel.Children.Count - 1]).Tag.ToString() == "Load More"))
                    {
                        ((PlatformCarouselItem)CarouselItemsPanel.Children[CarouselItemsPanel.Children.Count - 1]).Content = null;
                    }
                }
                ...
                {
                    if ((CarouselLinearPanel.Children.Count > 0 && (((PlatformCarouselItem)CarouselLinearPanel.Children[CarouselLinearPanel.Children.Count - 1]).Tag != null))
                        && (((PlatformCarouselItem)CarouselLinearPanel.Children[CarouselLinearPanel.Children.Count - 1]).Tag.ToString() == "Load More"))
                    {
                        ((PlatformCarouselItem)CarouselLinearPanel.Children[CarouselLinearPanel.Children.Count - 1]).Content = null;
                    }
                }
        ...
        private void OnLoadMoreViewChanged(DependencyPropertyChangedEventArgs e)
        {
            if (LoadMoreItemsCount > 0 && AllowLoadMore)
            {
                if (ViewMode == ViewMode.Linear)
                {
                    if (_carouselLinearPanel != null && ((PlatformCarouselItem)_carouselLinearPanel.Children[_carouselLinearPanel.Children.Count - 1]).Tag != null)
                    {
                        PlatformCarouselItem? carouselItem = _carouselLinearPanel.Children[_carouselLinearPanel.Children.Count - 1] as PlatformCarouselItem;
                        if (carouselItem != null)
                ...
                {
                    if (_carouselItemsPanel != null && ((PlatformCarouselItem)_carouselItemsPanel.Children[_carouselItemsPanel.Children.Count - 1]).Tag != null)
                    {
                        PlatformCarouselItem? carouselItem = _carouselItemsPanel.Children[_carouselItemsPanel.Children.Count - 1] as PlatformCarouselItem;
                        if (carouselItem != null)
                            carouselItem.Content = LoadMoreView;
                    }
                }
        ...
        void OnViewModeChanged(DependencyPropertyChangedEventArgs e)
        {
            ...
            else
            {
                ...
                ControlTemplate? defaultTemplate = rd["DefualtTemplate"] as ControlTemplate;
            ...
            if (ItemsSource is IEnumerable)
            {
                if (ItemsSource != null && AllowLoadMore)
                {
                    if (ItemsSource != null)
                    {
                        ...
                        int itemsToLoad = Math.Min(LoadMoreItemsCount, Queue.Count);

                        if(loadedItems > 0)
                        {
                            itemsToLoad = loadedItems;
                        }
                        if (itemsToLoad > Queue.Count)
                            itemsToLoad = Queue.Count;
                ...
                else if (EnableVirtualization)
                {
                    ...
                    TempCollection = ItemsSource as IEnumerable;
                    VirtualItemList.Clear();
                    var tempCollection = TempCollection as IList;
        ...
        void SetQueue()
        {
            ...
            foreach (var item in ((IEnumerable)ItemsSource))
            {
                ...
                {
                    contentView = GetItemView(virtualView, this, j);
                    j++;
                }
                if (contentView != null && _carouselHandler != null && virtualView != null)
                {
                    Queue.Enqueue(GetNativeViewItem(virtualView, contentView, _carouselHandler));
                }
            }
        }
        ...
        private void OnAllowLoadMoreChanged(DependencyPropertyChangedEventArgs e)
        {
            if (!AllowLoadMore && LoadMoreItemsCount == 0)
            {
                ...
                foreach (var item in ((IList)base.ItemsSource))
                {
                    collection.Add(item);
                }
        ...
        void CreateItemContent(int position, int index)
        {
            ...
            if (tempCollection != null)
            {
                var item = VirtualItemList[index];

                if (item != null && TabCollection != null)
                {
                    if (position != -1)
                    {
                        TabCollection.Insert(position, item);
                    }
                    else
                    {
        ...
        /// <param name="index"></param>
        /// <returns></returns>
        private object? GetVirtualizedItem(object? item, int index)
        {
            View? contentView = null;
            if (virtualView != null && virtualView.ItemTemplate != null)
            {
                contentView = GetItemView(virtualView, this, index);
            }
            if (contentView != null && _carouselHandler != null && virtualView != null)
            {
                return (GetNativeViewItem(virtualView, contentView, _carouselHandler));
            }
        ...
        void UpdateLinearStartEndPosition(double horizontalCenterPoint, double verticalCenterPoint, bool isUpdateStartPosition, double itemRegion, double startRegion, double endRegion)
        {
            if (TempCollection != null)
            {
                for (int i = 0; i < ((IList)TempCollection).Count; i++)
                {
        ...
        {
            _preEnd = _end;
            if ((!IsIntermediate && isRemove) || (IsIntermediate && !isRemove))
            {
                _preStart = _start;

                if (_start - itemCount > 0)
                {
                    _start = _start - itemCount;
                }
                else
                {
                    itemCount = itemCount + (_start - itemCount);
                    _start = 0;
                }
            }
        }
        ...
        {
            if (_preStart != 0 && VirtualItemList != null && TabCollection != null)
            {
                int firstIndex = ((IList)VirtualItemList).IndexOf(((IList)TabCollection)[0]);
        ...
        {
            if (TempCollection != null && _preEnd != ((IList)TempCollection).Count - 1 && TabCollection != null)
            {
                int lastIndex = ((IList)VirtualItemList).IndexOf(((IList)TabCollection)[(TabCollection as IList).Count - 1]);
                if (lastIndex == _end && itemCount != 0)
                {
                    if ((_preEnd + itemCount) == _end || _end == ((IList)TempCollection).Count - 1)
                    {
                        _preEnd = _end;
                        if ((_end + itemCount) > ((IList)TempCollection).Count - 1)
                        {
                            itemCount = itemCount - ((_end + itemCount) - (((IList)TempCollection).Count - 1));
                        }
                    }
                    else
                    {
        ...
        void AdjustEndIndex(int itemCount, bool isRemove)
        {
            ...
            if ((!IsIntermediate && isRemove) || (IsIntermediate && !isRemove))
            {
                ...
                if(TempCollection != null)
                {
                    ...
                    {
                        itemCount = itemCount - ((_end + itemCount) - (((IList)TempCollection).Count - 1));
                        _end = ((IList)TempCollection).Count - 1;
                    }
                }
            }
        }
        ...
        void DefaultViewModeIndex(Size size)
        {
            ...
            UpdateDefautStartEndPosition(horizontalCenterPoint, verticalCenterPoint, isUpdateStartPosition);
            UpdateItems();
        }
        ...
        /// </summary>
        void UpdateDefautStartEndPosition(double horizontalCenterPoint, double verticalCenterPoint, bool isUpdateStartPosition)
        {
            double hCenter = -1;
            double vCenter = -1;
            if (TempCollection != null)
        ...
        /// <summary>
        /// Reset revious start and end indices
        /// </summary>
            ...
            if (virtualView != null && virtualView.ItemsSource != null)
            {
                if (virtualView.ItemWidth > 0 && virtualView.ItemHeight > 0)
                {
                    ...
                    if (virtualView.ItemsSource != null)
                    {
                        ...
                        {
                            foreach (PlatformCarouselItem item in Items)
                            {
                                item.Content = null;
                            }
...
#pragma warning restore IDE0031
        ...
        /// <param name="index"></param>
        /// <returns></returns>
        internal View? GetItemView(ICarousel FormsCarousel, PlatformCarousel carousel, int index)
        {
            if ((FormsCarousel.ItemsSource is System.Collections.IList itemSource && itemSource.Count > 0))
            {
                if (FormsCarousel.ItemTemplate != null)
                {
                    ...
                    View? templateLayout = null;
                    if (FormsCarousel.ItemTemplate is Microsoft.Maui.Controls.DataTemplateSelector)
                    {
                        Microsoft.Maui.Controls.DataTemplateSelector? dataTemplateSelector = FormsCarousel.ItemTemplate as Microsoft.Maui.Controls.DataTemplateSelector;
                        if (FormsCarousel.ItemsSource != null && dataTemplateSelector != null)
                        {
                ...
                if (FormsCarousel.ItemTemplate == null)
                {
                    ...
                    templateLayout.VerticalTextAlignment = Microsoft.Maui.TextAlignment.Center;
                    templateLayout.HorizontalTextAlignment = Microsoft.Maui.TextAlignment.Center;
                    if (FormsCarousel.ItemsSource != null && FormsCarousel.ItemsSource.Count() > 0)
                    {
                        if (index < FormsCarousel.ItemsSource.Count())
        ...
        /// </summary>
        internal PlatformCarouselItem GetNativeViewItem(ICarousel virtualView, View contentView, ICarouselHandler handler)
        {
	...
	public class SfCarouselPanel : Canvas, IDisposable
    {
        ...
        internal void AddLoadMoreItem()
        {
            if (ParentItemsControl != null)
            {
                ...
                {
                    if ((Children.Count > 0 && (((PlatformCarouselItem)Children[Children.Count - 1]).Tag == null))
                    || (((PlatformCarouselItem)Children[Children.Count - 1]).Tag.ToString() != "Load More"))
                    {
                        if (ParentItemsControl.ItemsSource is IList itemSource && itemSource.Count > Children.Count)
                        {
                            if (ParentItemsControl.LoadMoreView == null)
                            {
                                ...
                                textBlock.Text = SfCarouselResources.LoadMoreText;
	...
	public class SfCarouselLinearPanel : Panel
    {
        ...
        /// </summary>
        private PlatformCarousel? parentItemsControl;
		...
		internal PlatformCarousel? ParentItemsControl
        {
            get
            {
                if (parentItemsControl == null)
                {
                    parentItemsControl = ItemsControl.GetItemsOwner(this) as PlatformCarousel;
                }
		...
		protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            {
                if (ParentItemsControl.SelectedIndex != -1 && !ParentItemsControl.IsDisableScrollChange)
                {
                    if (ParentItemsControl.VirtualItemList is IList virtualItemList && virtualItemList.Count > 0 && virtualItemList[ParentItemsControl.SelectedIndex] is PlatformCarouselItem item)
                    {
                        if (ParentItemsControl.Orientation == Orientation.Horizontal)
                        {
                ...
                else if (ParentItemsControl.IsDisableScrollChange && !ParentItemsControl.IsIntermediate)
                {
