		...
		void AddImageView()
		{
			if (_view != null)
			{
				if (!IsDynamic)
				{
					...
					{
						LayoutParameters = new LayoutParams((int)_width, (int)_height, GravityFlags.Center)
					};
					var bitmap = GetBitmapFromView(_view);
					if (bitmap != null)
					{
	...
	internal class ItemAdapter : RecyclerView.Adapter
	{
		...
		public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
		{
			...
			using (LinearLayout.LayoutParams imageParams = new LinearLayout.LayoutParams((int)_viewWidth, (int)_viewHeight))
			{
				...
				{
					if (_carousel.TempDataSource != null && position == _carousel.TempDataSource.Count - 1 && _carousel._tempItemsSource != null && _carousel.ItemsSourceCollection != null && _carousel.TempDataSource.Count > _carousel.ItemsSourceCollection.Count && _carousel.AllowLoadMore)
					{
						if (_carousel.LoadMoreView == null)
						{
							_passingView = LoadMoreDefaultView;
						}
						else
						{
		...
		public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
		{
			...
			{
				LayoutParameters = new ViewGroup.LayoutParams((int)_viewWidth, (int)_viewHeight)
			};
			frame.RequestLayout();
		...
		{
			if (holder is ViewHolder viewHolder && viewHolder.ItemView != null)
			{
		...
		internal void LoadMore(int index = 0)
		{
			...
			if (_carousel != null && _carousel.LoadMoreItemsCount > 0 && (_carousel.TempDataSource != null && ((_carousel.AllowLoadMore && index == _carousel.TempDataSource.Count - 1) || (!_carousel.AllowLoadMore && index == _carousel.TempDataSource.Count))) && _carousel.AllowLoadMoreDataSource != null && _carousel.AllowLoadMoreDataSource.Count > 0)
			{
				...
				if (_carousel.AllowLoadMoreDataSource != null)
				{
					...
					if (_carousel.ItemsSourceCollection != null)
					{
						_carousel._countItem = _carousel.ItemsSourceCollection.Count;
					}
		...
		void CustomImageView_Click(object? sender, EventArgs e)
		{
			if (_carousel != null)
			{
				_carousel._isImageClicked = true;

				if (sender is CustomImageView imageView)
				{
