...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
	public class SfTextInputLayoutUnitTests : BaseUnitTest
	{
		...
		#region Handler Lifecycle Tests (Issue #329 — iOS hint text disappears after modal close/reopen)

		/// <summary>
		/// Verifies that when the handler is detached (Handler == null), e.g., on modal dismiss on iOS,
		/// OnHandlerChanged clears all three label styles to null.
		/// This is the "before" state that the fix for issue #329 must recover from.
		/// </summary>
		[Fact]
		public void OnHandlerChanged_WhenHandlerIsNull_SetsAllLabelStylesToNull()
		{
			var inputLayout = new SfTextInputLayout();

			// All three styles are initialised by the bindable property default value creators.
			Assert.NotNull(inputLayout.HintLabelStyle);
			Assert.NotNull(inputLayout.HelperLabelStyle);
			Assert.NotNull(inputLayout.ErrorLabelStyle);

			// In unit tests, Handler is always null, so OnHandlerChanged triggers the else branch
			// which nulls all three label styles — reproducing the problematic "after modal close" state.
			InvokePrivateMethod(inputLayout, "OnHandlerChanged");

			Assert.Null(inputLayout.HintLabelStyle);
			Assert.Null(inputLayout.HelperLabelStyle);
			Assert.Null(inputLayout.ErrorLabelStyle);
		}

		/// <summary>
		/// Verifies that GetHintLabelStyleDefaultValue returns a non-null LabelStyle
		/// with the correct default font size (16).
		/// This method is called by the fix to reinitialise a null HintLabelStyle on handler reattach.
		/// </summary>
		[Fact]
		public void GetHintLabelStyleDefaultValue_ReturnsNonNullWithDefaultFontSize()
		{
			var inputLayout = new SfTextInputLayout();

			var style = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetHintLabelStyleDefaultValue");
		...
		/// <summary>
		/// Verifies that GetHelperLabelStyleDefaultValue returns a non-null LabelStyle.
		/// </summary>
		[Fact]
		public void GetHelperLabelStyleDefaultValue_ReturnsNonNull()
		{
			var inputLayout = new SfTextInputLayout();

			var style = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetHelperLabelStyleDefaultValue");

			Assert.NotNull(style);
		}

		/// <summary>
		/// Verifies that GetErrorLabelStyleDefaultValue returns a non-null LabelStyle.
		/// </summary>
		[Fact]
		public void GetErrorLabelStyleDefaultValue_ReturnsNonNull()
		{
			var inputLayout = new SfTextInputLayout();

			var style = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetErrorLabelStyleDefaultValue");

			Assert.NotNull(style);
		}

		/// <summary>
		/// Simulates the full detach → reinit cycle that the fix for issue #329 addresses:
		/// label styles are explicitly set to null (handler detach / modal closed), then
		/// re-assigned via the GetXxxLabelStyleDefaultValue helpers (handler reattach / modal reopened).
		/// Verifies all three styles become non-null after reinitialisation.
		/// </summary>
		[Fact]
		public void LabelStyles_WhenNulledThenReinit_AreRestoredToNonNull()
		{
			var inputLayout = new SfTextInputLayout();

			// Simulate what the else-branch in OnHandlerChanged does (modal dismissed / handler detached).
			inputLayout.HintLabelStyle = null;
			inputLayout.HelperLabelStyle = null;
			inputLayout.ErrorLabelStyle = null;

			Assert.Null(inputLayout.HintLabelStyle);
			Assert.Null(inputLayout.HelperLabelStyle);
			Assert.Null(inputLayout.ErrorLabelStyle);

			// Simulate the fix in the Handler != null branch (modal reopened / handler reattached):
			// only reinitialise styles that are null.
			if (inputLayout.HintLabelStyle == null)
			{
				inputLayout.HintLabelStyle = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetHintLabelStyleDefaultValue");
			}

			if (inputLayout.HelperLabelStyle == null)
			{
				inputLayout.HelperLabelStyle = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetHelperLabelStyleDefaultValue");
			}

			if (inputLayout.ErrorLabelStyle == null)
			{
				inputLayout.ErrorLabelStyle = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetErrorLabelStyleDefaultValue");
			}
		...
		/// <summary>
		/// Verifies that a user-provided label style that is already non-null is NOT replaced
		/// by the reinit logic in the fix. The null-guard ("if (xxx == null)") ensures user
		/// customisations survive the detach → reattach cycle.
		/// </summary>
		[Fact]
		public void HintLabelStyle_WhenCustomStyleIsNotNull_IsPreservedByReinitLogic()
		{
			var inputLayout = new SfTextInputLayout();
			var customStyle = new LabelStyle { FontSize = 20 };
			inputLayout.HintLabelStyle = customStyle;

			// Apply the reinit guard from the fix: only reinitialise if null.
			if (inputLayout.HintLabelStyle == null)
			{
				inputLayout.HintLabelStyle = (LabelStyle)InvokeStaticPrivateMethod(inputLayout, "GetHintLabelStyleDefaultValue");
			}

			// Custom style must be untouched because it was not null.
			Assert.Same(customStyle, inputLayout.HintLabelStyle);
			Assert.Equal(20, inputLayout.HintLabelStyle!.FontSize);
		}
