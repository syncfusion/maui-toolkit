...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
	public class DataLabelUnitTests : BaseUnitTest
	{
		...
		public void GetAutoLabelPosition_ColumnSeries()
		{
			...
			PointF result = ChartDataLabelSettings.GetAutoLabelPosition(columnSeries, x, y, labelSize, padding, borderWidth);
		...
		public void IsTopWithLabelIndex_ShouldReturnTrueForTopValue()
		{
			...
			{
				YValues = [10, 14, 30, 10, 40, 10, 50]
			};
			int index = 2;
			areaSeries.ShowDataLabels = true;
			var cartesianDataLabelSettings = new CartesianDataLabelSettings();
			areaSeries.DataLabelSettings = cartesianDataLabelSettings;
			var result = (bool?)InvokePrivateMethod(cartesianDataLabelSettings, "IsTopWithLabelIndex", [areaSeries, index]);
			Assert.True(result);
		}
		...
		public void GetLabelPositionForRangeSeries()
		{
			...
			RangeAreaSeries RangeAreaSeries = new RangeAreaSeries()
			{
				ActualYAxis = new NumericalAxis(),
				ActualXAxis = new NumericalAxis(),
				HighValues = [10, 14, 30, 10, 40, 10, 50],
				LowValues = [40, 40, 40, 40, 40, 40, 40, 40],
			...
			var result = (PointF?)methodInfo?.Invoke(cartesianDataLabelSettings, [RangeAreaSeries, 0, labelSize, labelPoint, padding, valueType]);
			Assert.Equal(expectedResult, result);
		}
		...
		public void GetLabelPositionForCandleSeries()
		{
			...
			CandleSeries candleSeries = new CandleSeries()
			{
				...
				ActualXAxis = new NumericalAxis(),
				HighValues = [10, 14, 30, 10, 40, 10, 50],
				LowValues = [30, 30, 30, 30, 30, 30, 30],
				CloseValues = [40, 40, 40, 40, 40, 40, 40, 40],
				OpenValues = [50, 50, 50, 50, 50, 50, 50],
			...
			var result = (PointF?)methodInfo?.Invoke(cartesianDataLabelSettings, [candleSeries, 0, labelSize, labelPoint, padding, valueType]);
			Assert.Equal(expectedResult, result);
		}
		...
		public void UpdateHighTypePosition()
		{
			...
			object[] parameters = [highValue, lowValue, x, y, labelSize, padding, 0, candleSeries];
		...
		public void UpdateOpenTypePosition()
		{
			...
			object[] parameters = [openValue, closeValue, x, y, labelSize, padding, 0, candleSeries];
			methodInfo?.Invoke(cartesianDataLabelSettings, parameters);
			x = (float)parameters[2];
			y = (float)parameters[3];
			Assert.Equal(expectedX, x);
