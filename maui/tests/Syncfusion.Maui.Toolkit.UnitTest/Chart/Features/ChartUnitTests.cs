...
namespace Syncfusion.Maui.Toolkit.UnitTest
{
	public class ChartUnitTests : BaseUnitTest
	{
		...
		public void ValueToPoint_AxisIsHorizontal()
		{
			...
			var sfCartesianChart = new SfCartesianChart();
			sfCartesianChart._chartArea.ActualSeriesClipRect = new Rect(30, 1, 1007.599975, 471.79998);
		...
		public void GetTooltipInfo_ShouldReturnTooltip()
		{
			...
			{
				AreaBounds = new Rect(0, -1, 1037.5999755859375, 504.79998779296875)
			};
			float[] valuesFromImage =
				[
		...
		public void ViewAnnotationAttachedToChart_WithXamlData_AttachesSuccessfully()
		{
			...
			Assert.Contains(viewAnnotation.View, sfCartesianChart._annotationLayout.Children);
		...
		public void ViewAnnotationDetachedToChart_RemovesSuccessfully()
		{
			...
			sfCartesianChart._annotationLayout.Children.Add(viewAnnotation.View);
			sfCartesianChart.ViewAnnotationDetachedToChart(viewAnnotation);
			Assert.DoesNotContain(viewAnnotation.View, sfCartesianChart._annotationLayout.Children);
		}
		...
		public void SolveQuadraticEquation_WithRealRoots_ReturnsTrue()
		{
			...
			double b = 1;
			double c = -6;
			var result = (bool?)InvokePrivateMethod(sfPyramidChart, "SolveQuadraticEquation", [b, c]);
			Assert.True(result);
		}
		...
		public void RemoveData_DecreasesDataPoints()
		{
			...
			_ = InvokePrivateMethod(sfPyramidChart, "RemoveData", [index]);
		...
		public void ResetData_ClearsDataAndResetsPointsCount()
		{
			...
			var actualData = new List<object> { new(), new(), new() };
			var yValues = new List<double> { 10, 20, 30 };
			...
			_ = InvokePrivateMethod(sfPyramidChart, "ResetData", Array.Empty<object>());
		...
		public void ResetDataPoint_WithItemsSource_GeneratesDataPoints()
		{
			...
			var actualData = new List<object> { new(), new() };
			...
			_ = InvokePrivateMethod(sfPyramidChart, "ResetDataPoint", Array.Empty<object>());
			Assert.Equal(0, GetPrivateField(sfPyramidChart, "_pointsCount"));
		}
		...
		public void CalculateLinearSegment()
		{
			...
			InvokePrivateMethod(sfPyramidChart, "CalculateLinearSegment", [5, 0.5, new List<double> { 5 }]);
		...
		public void CalculateSurfaceSegment()
		{
			...
			{ GapRatio = 0.5, };
			SetPrivateField(sfPyramidChart, "_pointsCount", 1);
			InvokePrivateMethod(sfPyramidChart, "CalculateSurfaceSegment", [5, 0.5, new List<double> { 5 }]);
			var result = (ObservableCollection<ChartSegment>?)GetPrivateField(sfPyramidChart, "_segments");
			Assert.True((result?.Count >= 1));
		}
		...
		public void GetActualXValue_NullXValues_ReturnsNull()
		{
			...
			int index = 2;
			SetPrivateField(sfPyramidChart, "_pointsCount", -1);
			var result = InvokePrivateMethod(sfPyramidChart, "GetActualXValue", [index]);
			Assert.Null(result);
		}
		...
		public void GetActualXValue_XValues_ReturnsNull()
		{
			...
			SetPrivateField(sfPyramidChart, "_pointsCount", 5);
			long expected = 30;
			var result = (long?)InvokePrivateMethod(sfPyramidChart, "GetActualXValue", [index]);
			Assert.Equal(expected, result);
		}
		...
		public void GetPropertyValue_ValidPath_ReturnsCorrectValue()
		{
			...
			var result = InvokeStaticPrivateMethodClass(typeof(SfPyramidChart), "GetPropertyValue", [person, paths]);
		...
		public void ReflectedObject_ValidPath_ReturnsCorrectValue()
		{
			...
			var result = InvokeStaticPrivateMethodClass(typeof(SfPyramidChart), "ReflectedObject", [person.Address, "City"]);
			Assert.Equal("New York", result);
		}
		...
		public void ReflectedObject_InvalidPath_ReturnsNull()
		{
			var address = new Address { City = "New York" };
			var person = new Person { Address = address };
			var result = InvokeStaticPrivateMethodClass(typeof(SfPyramidChart), "ReflectedObject", [person.Address, "NonExistentProperty"]);
			Assert.Null(result);
		}
		...
		public void GetArrayPropertyValue_ValidPath_ReturnsCorrectValue()
		{
			...
			var person = new Person { Addresses = [address1, address2] };

			string[] paths = ["Addresses[1]", "City"];
			var result = InvokeStaticPrivateMethodClass(typeof(SfPyramidChart), "GetArrayPropertyValue", [person, paths]);
		...
		{
			var address1 = new Address { City = "New York" };
			var person = new Person { Addresses = [address1] };
			string[] paths = ["Addresses[1]", "City"];
			var result = InvokeStaticPrivateMethodClass(typeof(SfPyramidChart), "GetArrayPropertyValue", [person, paths]);
			Assert.Null(result);
		}
		...
		public void GeneratePropertyPoints_ChartValueTypeDouble_StoresCorrectPoints()
		{
			...
			var itemsSource = new List<ModelTestCase>
				{
					new() { XValue = 10.0, YValue = 100.0 },
					new() { XValue = 20.0, YValue = 200.0 },
					new() { XValue = 30.0, YValue = 300.0 },
				};
			sfPyramidChart.ItemsSource = itemsSource;
			...
			InvokePrivateMethod(sfPyramidChart, "GeneratePropertyPoints", [yPaths, yLists]);
			...
			Assert.Equal([100.0, 200.0, 300.0], yValues);
		...
		public void SetIndividualPoint_UpdatesValuesCorrectly()
		{
			...
			var itemsSource = new List<ModelTestCase>
			{
				new() { XValue = 1.0, YValue = 100.0 },
				new() { XValue = 2.0, YValue = 200.0 }
			};
			...
			SetPrivateField(sfPyramidChart, "_isLinearData", true);
			InvokePrivateMethod(sfPyramidChart, "SetIndividualPoint", [0, new ModelTestCase { XValue = 3.0, YValue = 250.0 }, false]);
			var xValues = GetPrivateField(sfPyramidChart, "_xValues");
		...
		public void SetIndividualPoint_ReplacesValuesCorrectly()
		{
			...
			SetPrivateField(sfPyramidChart, "_pointsCount", 2);
			SetPrivateField(sfPyramidChart, "_isLinearData", true);
			InvokePrivateMethod(sfPyramidChart, "SetIndividualPoint", [0, new ModelTestCase { XValue = 0.5, YValue = 150.0 }, true]);
			var xValues = GetPrivateField(sfPyramidChart, "_xValues");
			var yValues = GetPrivateField(sfPyramidChart, "_yValues");
			var actualData = GetPrivateField(sfPyramidChart, "_pointsCount");
