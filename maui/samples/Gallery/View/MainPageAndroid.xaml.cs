...
public partial class MainPageAndroid : ContentPage
{
    ...
    private static bool HandleSampleBackNavigation()
    {
        if(mainPage != null)
        {
            if (mainPage.IsSampleLoadedByFilter)
            {
    ...
    private void LoadSamplePage(ControlModel controlModel, SampleCategoryModel? sampleCategoryModel = null)
    {
        ...
        this.chipScroll?.ScrollToAsync(0, 0, true);
    ...
    private async void Category_Tapped(object sender, EventArgs e)
    {
        ...
        if (previousSampleCategoryModel != null && sampleCategoryModel != null)
        {
            if (previousSampleCategoryModel == sampleCategoryModel)
            {
                return;
            }
    ...
    internal void LoadSample(SampleModel? sampleModel, CustomCardLayout? parent = null, bool isPopUpView = false, bool isCardLayout = false)
    {
        ...
        try
        {
            ...
            if (loadedSample != null)
            {
                loadedSample.IsCardView = isCardLayout;
    ...
    private void UpdateUIElementVisibility(VisualElement element, string? data, ColumnDefinition? columnWidth = null, double widthValue = 0)
    {
    ...
    private void ResetSettings()
    {
        ...
        this.youtubeIconGrid.IsVisible = false;
        this.sourceLinkGrid.IsVisible=false;

        this.optionColumnWidth.Width = 0;
        this.youtubeColumnWidth.Width = 0;
        this.sourceColumnWidth.Width = 0;
        this.whatsNewColumnWidth.Width = 0;
    ...
    private void TapGestureRecognizer_Tapped(object sender, EventArgs e)
    {
        ...
        this.searchedSampleSettingsImage.IsVisible = false;
        this.searchedSampleCodeViewerImage.IsVisible = false;
    ...
    {
        if (this.IsSampleLoadedByFilter)
        {
            if (this.allSamples.IsChecked == true)
            {
                this.NavigateBackToFilterList();
            }
            else
            {
	...
	{
        noneOption.IsChecked = noneOption.IsChecked ? false : true;
	}
	...
	{
        ascending.IsChecked = ascending.IsChecked ? false : true;
	}
	...
	{
        descending.IsChecked = descending.IsChecked ? false : true;
	}
